...
const Login = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className=" flex flex-col gap-4 m-auto items-start p-8 py-12 w-80 sm:w-[352px] text-gray-500 rounded-lg shadow-xl border border-gray-200 bg-white">
      ...
      {state === "register" ? (
        <p>
          Already have account? <span onClick={() => setState("login")} className="outline-purple-700 cursor-pointer text-purple-700">click here</span>
        </p>
      ) : (
        <p>
          Create an account? <span onClick={() => setState("register")} className="outline-purple-700 cursor-pointer text-purple-700">click here</span>
        </p>
      )}
